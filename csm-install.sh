...
_setup_variables() {
    ...
    # Create the file from the repo template if it does not exist yet
    if [[ ! -f "$csm_ini_file" ]]; then
        local csm_ini_temp="${script_dir}/../csm.ini"
        if [[ -f "$csm_ini_temp" ]]; then
            cp "$csm_ini_temp" "$csm_ini_file"
            _log STEP "Copied default config template to $csm_ini_file"
        else
            # Fallback: create a csm.ini file with the minimum required vars
...
EOF
    ...
    # Load user overrides – these take precedence over the defaults
    local user_conf="${script_dir}/user.conf"
    if [[ -f "$user_conf" ]]; then
        source "$user_conf"
        _log PASS "Loaded user overrides from file: $user_conf"
    fi

    # Optional interactive override – only when not forcing
    if [[ $(_confirm_no "Do you want to manually set each variable? No uses default values.") ]] ; then
        if [[ $force_install -eq 0 ]]; then
            _log INFO "You may override any default value below. Press ENTER to keep the current value."
            local var cur new
            for var in "${csm_vars[@]}"; do
                cur="${!var}"
                read -r -p "${ylw}${bld}Current value for '${rst}${var}${ylw}': ${rst}${cur}${rst}" new
                if [[ -n "$new" ]]; then
                    # Update the variable in the environment
                    export "$var=$new"
                    # And persist it in the user.conf file
                    if [[ -f "$user_conf" ]]; then
                        sed -i "s|^${var}=.*|${var}=${new}|" "$user_conf"
                    else
                        echo "${var}=${new}" >>"$user_conf"
                    fi
                    _log STEP "Updated $var, old: $cur -> $new"
                fi
            done
        fi
    fi
    ...
    for var in "${csm_vars[@]}"; do export "$var"; done
}
